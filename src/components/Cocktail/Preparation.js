import React from "react";

function Preparation({ steps }) {
    return (
        <div className="md:w-1/2 space-y-4 mt-6 md:mt-0">
            <h3 className="font-medium text-2xl" style={{ color: '#000000', fontFamily: 'SFProRegular' }}>Preparation</h3>
            <div className="space-y-4">
                {steps.map((step, index) => (
                    <div key={index} className="border border-gray-400 rounded p-4">
                        <span className="font-bold text-lg" style={{ color: '#FF2D55', fontFamily: 'SFProRegular' }}>
                            {index + 1}.
                        </span>
                        <span className="ml-2 text-lg" style={{ color: '#000000', fontFamily: 'SFProRegular' }}>
                            {step} {/* Directly using the step string */}
                        </span>
                    </div>
                ))}
            </div>
        </div>
    );
}

export default Preparation;
